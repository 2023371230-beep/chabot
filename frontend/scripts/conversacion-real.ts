...
/** El estado ahora vive en la base, asi que limpiar es borrar filas. */
const limpiarEstado = async (): Promise<void> => {
...
const main = async (): Promise<void> => {
  await limpiarEstado();
  const gastoAntes = await estadoPresupuesto();
  ...
  const gasto = await estadoPresupuesto();
  const usadas =
    gasto.disponible && gastoAntes.disponible
      ? (gasto.usadasHoy ?? 0) - (gastoAntes.usadasHoy ?? 0)
      : 0;
  console.log('');
  console.log('  ' + '='.repeat(74));
  console.log(`  Mensajes en la conversacion:   ${conversacion.length}`);
  console.log(`  Peticiones de IA consumidas:   ${usadas}`);
  console.log(`  Resueltos sin IA:              ${conversacion.length - usadas}`);
